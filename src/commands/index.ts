...
export function registerCommands(bot: Bot<BotContext>): void {
  bot.command('start', async (ctx) => {
    ...
    const memberCount = await ctx.services.households.memberCount(household.id);
    if (memberCount >= 2) {
      await ctx.reply(`👋 Welcome back.\n\nHousehold: ${household.name}\nBoth MVP seats are already connected. Use /center to open Weekly Financial Center.`, { reply_markup: centerKeyboard() });
      return;
    }
  ...
  bot.command('invite', async (ctx) => {
    const user = await ensureUser(ctx);
    const invite = await ctx.services.households.createInvite(user.id);
    await ctx.reply(`🔗 Invite your second household member.

Code: \`${invite.code}\`
Expires: ${format(invite.expiresAt, 'd MMM yyyy')}

Share: /start invite_${invite.code}`, { parse_mode: 'Markdown' });
  });
