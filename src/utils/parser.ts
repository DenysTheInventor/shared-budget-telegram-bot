...
export function parseBudgetMessage(text: string, now = new Date()): ParsedBudgetMessage {
  ...
  const recurringFrequency: ParsedBudgetMessage['recurringFrequency'] = normalized.includes('monthly') ? 'MONTHLY' : normalized.includes('weekly') ? 'WEEKLY' : undefined;
  ...
  const intent: ParsedBudgetMessage['intent'] = /salary|income|paid me|bonus/.test(normalized) || categoryName === 'salary' ? 'ADD_INCOME' : 'ADD_EXPENSE';
